  ...
  try {
    ...
    if (!announcementsSnap.empty) {
      ...
      for (const userDoc of allUsersSnap.docs) {
        ...
        if (user.email) {
          try {
            await fetch(`${API_BASE_URL}/api/send-community-update`, {
              method: 'POST',
      ...
      if (!isProfileComplete) {
        ...
        if (daysSinceJoined >= 2 && reminderCount === 0) {
          try {
            ...
            await fetch(`${API_BASE_URL}/api/send-profile-reminder`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
         ...
         if (nudgeCount === 0) {
             ...
             if (daysSinceJoined >= 1) {
                 try {
                    ...
                    await fetch(`${API_BASE_URL}/api/send-resource-nudge`, {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({ message: 'Automation run complete', emailsSent })
    };
